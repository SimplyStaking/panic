...
class TestSystemAlertsConfigsFactory(unittest.TestCase):
    """
    Although currently there is only one type of system alerts config, the tests
    ...
    def setUp(self) -> None:
        ...
        self.test_chain_name_1 = 'general'
        self.test_chain_name_2 = 'cosmos cosmoshub'

        """
        First we will construct the received alerts configurations.
        """

        system_config_metrics = [
            'open_file_descriptors', 'system_cpu_usage',
            'system_storage_usage', 'system_ram_usage', 'system_is_down'
        ]
        self.received_config_example_1_system = {}
        self.received_config_example_2_system = {}

        for i in range(len(system_config_metrics)):
            self.received_config_example_1_system[str(i)] = {
                'name': system_config_metrics[i],
                'parent_id': self.test_parent_id_1
            }
            self.received_config_example_2_system[str(i)] = {
                'name': system_config_metrics[i],
        ...
        filtered_1_system = {}
        for _, config in self.received_config_example_1_system.items():
            filtered_1_system[config['name']] = copy.deepcopy(config)

        filtered_2_system = {}
        for _, config in self.received_config_example_2_system.items():
            filtered_2_system[config['name']] = copy.deepcopy(config)

        self.alerts_config_1_system = SystemAlertsConfig(
            parent_id=self.test_parent_id_1,
            open_file_descriptors=filtered_1_system['open_file_descriptors'],
            system_cpu_usage=filtered_1_system['system_cpu_usage'],
            system_storage_usage=filtered_1_system['system_storage_usage'],
            system_ram_usage=filtered_1_system['system_ram_usage'],
            system_is_down=filtered_1_system['system_is_down']
        )
        self.alerts_config_2_system = SystemAlertsConfig(
            parent_id=self.test_parent_id_2,
            open_file_descriptors=filtered_2_system['open_file_descriptors'],
            system_cpu_usage=filtered_2_system['system_cpu_usage'],
            system_storage_usage=filtered_2_system['system_storage_usage'],
            system_ram_usage=filtered_2_system['system_ram_usage'],
            system_is_down=filtered_2_system['system_is_down']
        )

        self.system_configs_factory = SystemAlertsConfigsFactory()

    def tearDown(self) -> None:
        self.received_config_example_1_system = None
        self.received_config_example_2_system = None
        self.alerts_config_1_system = None
        self.alerts_config_2_system = None
        self.system_configs_factory = None

    @parameterized.expand([
        ('self.system_configs_factory',
         'self.received_config_example_1_system',
         'self.alerts_config_1_system',
         'self.received_config_example_2_system',
         'self.alerts_config_2_system',)
    ])
    ...
    @parameterized.expand([
        ('self.system_configs_factory', 'self.alerts_config_1_system',
         'self.alerts_config_2_system',)
    ])
