...
class TestPagerDutyAlertsHandler(unittest.TestCase):
    def setUp(self) -> None:
        self.test_handler_name = 'test_pagerduty_alerts_handler'
        ...
        self.test_api = PagerDutyApi(self.test_integration_key)
        self.test_channel = PagerDutyChannel(self.test_channel_name,
        ...
        self.test_pagerduty_alerts_handler = PagerDutyAlertsHandler(
    ...
    def tearDown(self) -> None:
        ...
        connect_to_rabbit(self.test_pagerduty_alerts_handler.rabbitmq)
        delete_queue_if_exists(self.test_pagerduty_alerts_handler.rabbitmq,
                               self.test_rabbit_queue_name)
        delete_queue_if_exists(
            self.test_pagerduty_alerts_handler.rabbitmq,
            self.test_pagerduty_alerts_handler._pagerduty_alerts_handler_queue)
        delete_exchange_if_exists(self.test_pagerduty_alerts_handler.rabbitmq,
                                  HEALTH_CHECK_EXCHANGE)
        delete_exchange_if_exists(self.test_pagerduty_alerts_handler.rabbitmq,
                                  ALERT_EXCHANGE)
        disconnect_from_rabbit(self.test_pagerduty_alerts_handler.rabbitmq)
        ...
        self.test_api = None
        self.test_pagerduty_alerts_handler = None
        self.test_alerts_queue = None

    def test__str__returns_handler_name(self) -> None:
        self.assertEqual(self.test_handler_name,
                         str(self.test_pagerduty_alerts_handler))

    def test_handler_name_returns_handler_name(self) -> None:
        self.assertEqual(self.test_handler_name,
                         self.test_pagerduty_alerts_handler.handler_name)

    @mock.patch.object(RabbitMQApi, "start_consuming")
    def test_listen_for_data_calls_start_consuming(
            self, mock_start_consuming) -> None:
        mock_start_consuming.return_value = None
        self.test_pagerduty_alerts_handler._listen_for_data()
        mock_start_consuming.assert_called_once_with()

    def test_pagerduty_channel_returns_associated_pagerduty_channel(
            self) -> None:
        self.assertEqual(self.test_channel,
                         self.test_pagerduty_alerts_handler.pagerduty_channel)

    def test_alerts_queue_returns_the_alerts_queue(self) -> None:
        self.test_pagerduty_alerts_handler._alerts_queue = \
            self.test_alerts_queue
        self.assertEqual(self.test_alerts_queue,
                         self.test_pagerduty_alerts_handler.alerts_queue)
    ...
    def test_init_initialises_handler_correctly(self) -> None:
        ...
        self.assertEqual(
            ...
            self.test_pagerduty_alerts_handler.alerts_queue.maxsize)
        self.assertEqual(self.test_max_attempts,
                         self.test_pagerduty_alerts_handler._max_attempts)
        self.assertEqual(
    ...
    def test_initialise_rabbitmq_initialises_rabbit_correctly(
        ...
        try:
            ...
            self.test_pagerduty_alerts_handler._initialise_rabbitmq()
            ...
            # called successfully.
            self.assertTrue(
                self.test_pagerduty_alerts_handler.rabbitmq.is_connected)
            self.assertTrue(
                self.test_pagerduty_alerts_handler.rabbitmq.connection.is_open)
            self.assertTrue(
    ...
    def test_send_heartbeat_sends_a_heartbeat_correctly(self) -> None:
        ...
        try:
            ...
            self.assertEqual(0, res.method.message_count)
            self.test_pagerduty_alerts_handler.rabbitmq.queue_bind(
                queue=self.test_rabbit_queue_name,
                exchange=HEALTH_CHECK_EXCHANGE,
                routing_key=HEARTBEAT_OUTPUT_WORKER_ROUTING_KEY)

            self.test_pagerduty_alerts_handler._send_heartbeat(
                self.test_heartbeat)
            ...
            _, _, body = self.test_pagerduty_alerts_handler.rabbitmq.basic_get(
                self.test_rabbit_queue_name)
    ...
    @mock.patch.object(PagerDutyAlertsHandler, "_send_alerts")
    @mock.patch.object(PagerDutyAlertsHandler, "_place_alert_on_queue")
    ...
    def test_process_alert_places_data_on_queue_if_no_processing_errors(
        ...
        try:
            ...
            # Send alert
            self.test_pagerduty_alerts_handler._process_alert(blocking_channel,
                                                              method,
                                                              properties, body)
    ...
    def test_process_alert_raises_exception_if_sending_data_from_queue_error(
        ...
        try:
            ...
            self.assertRaises(error_class,
                              self.test_pagerduty_alerts_handler._process_alert,
    ...
    @mock.patch.object(Queue, "empty")
    @mock.patch.object(PagerDutyAlertsHandler, "_send_heartbeat")
    @mock.patch.object(PagerDutyAlertsHandler, "_send_alerts")
    @mock.patch.object(PagerDutyAlertsHandler, "_place_alert_on_queue")
    @mock.patch.object(RabbitMQApi, "basic_ack")
    ...
    def test_place_alert_on_queue_places_alert_on_queue_if_queue_not_full(
        ...
        self.test_pagerduty_alerts_handler._place_alert_on_queue(
            self.test_alert)
    ...
    @freeze_time("2012-01-01")
    @mock.patch.object(PagerDutyChannel, "alert")
    def test_send_alerts_discards_old_alerts_and_sends_the_recent(
        ...
        self.assertTrue(self.test_pagerduty_alerts_handler.alerts_queue.empty())
    ...
    def test_send_alerts_stops_sending_if_an_alert_is_not_successfully_sent(
        ...
        self.assertEqual(
            2, self.test_pagerduty_alerts_handler.alerts_queue.qsize())
        self.assertEqual(
            test_alert_1,
            self.test_pagerduty_alerts_handler.alerts_queue.queue[0])
        self.assertEqual(
            test_alert_2,
            self.test_pagerduty_alerts_handler.alerts_queue.queue[1])
