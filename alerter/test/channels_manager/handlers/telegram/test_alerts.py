...
class TestTelegramAlertsHandler(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def tearDown(self) -> None:
        ...
        connect_to_rabbit(self.test_telegram_alerts_handler.rabbitmq)
        delete_queue_if_exists(self.test_telegram_alerts_handler.rabbitmq,
                               self.test_rabbit_queue_name)
        delete_queue_if_exists(
            self.test_telegram_alerts_handler.rabbitmq,
            self.test_telegram_alerts_handler._telegram_alerts_handler_queue)
        delete_exchange_if_exists(self.test_telegram_alerts_handler.rabbitmq,
                                  HEALTH_CHECK_EXCHANGE)
        delete_exchange_if_exists(self.test_telegram_alerts_handler.rabbitmq,
                                  ALERT_EXCHANGE)
        disconnect_from_rabbit(self.test_telegram_alerts_handler.rabbitmq)
    ...
    def test_telegram_channel_returns_associated_telegram_channel(
        ...
        self.assertEqual(self.test_channel,
                         self.test_telegram_alerts_handler.telegram_channel)

    def test_alerts_queue_returns_the_alerts_queue(self) -> None:
        self.test_telegram_alerts_handler._alerts_queue = self.test_alerts_queue
        self.assertEqual(self.test_alerts_queue,
                         self.test_telegram_alerts_handler.alerts_queue)
    ...
    def test_init_initialises_handler_correctly(self) -> None:
        ...
        self.assertEqual(
            ...
            self.test_telegram_alerts_handler.alerts_queue.maxsize)
        self.assertEqual(self.test_max_attempts,
                         self.test_telegram_alerts_handler._max_attempts)
        self.assertEqual(
            self.test_alert_validity_threshold,
            self.test_telegram_alerts_handler._alert_validity_threshold)
        ...
        self.assertEqual(CHANNEL_HANDLER_INPUT_ROUTING_KEY_TEMPLATE.format(
            self.test_channel_id),
            self.test_telegram_alerts_handler._telegram_channel_routing_key)
    ...
    def test_initialise_rabbitmq_initialises_rabbit_correctly(
        ...
        try:
            ...
            # called successfully.
            self.assertTrue(
                self.test_telegram_alerts_handler.rabbitmq.is_connected)
            self.assertTrue(
                self.test_telegram_alerts_handler.rabbitmq.connection.is_open)
            self.assertTrue(
    ...
    def test_send_heartbeat_sends_a_heartbeat_correctly(self) -> None:
        ...
        try:
            ...
            self.assertEqual(0, res.method.message_count)
            self.test_telegram_alerts_handler.rabbitmq.queue_bind(
                queue=self.test_rabbit_queue_name,
                exchange=HEALTH_CHECK_EXCHANGE,
                routing_key=HEARTBEAT_OUTPUT_WORKER_ROUTING_KEY)

            self.test_telegram_alerts_handler._send_heartbeat(
                self.test_heartbeat)
            ...
            _, _, body = self.test_telegram_alerts_handler.rabbitmq.basic_get(
                self.test_rabbit_queue_name)
