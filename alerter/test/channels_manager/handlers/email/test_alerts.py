...
from src.utils.constants.rabbitmq import (
    HEALTH_CHECK_EXCHANGE, ALERT_EXCHANGE, HEARTBEAT_OUTPUT_WORKER_ROUTING_KEY,
...
class TestEmailAlertsHandler(unittest.TestCase):
    ...
    def tearDown(self) -> None:
        ...
        connect_to_rabbit(self.test_email_alerts_handler.rabbitmq)
        delete_queue_if_exists(self.test_email_alerts_handler.rabbitmq,
                               self.test_rabbit_queue_name)
        delete_queue_if_exists(
            self.test_email_alerts_handler.rabbitmq,
            self.test_email_alerts_handler._email_alerts_handler_queue)
        delete_exchange_if_exists(self.test_email_alerts_handler.rabbitmq,
                                  HEALTH_CHECK_EXCHANGE)
        delete_exchange_if_exists(self.test_email_alerts_handler.rabbitmq,
                                  ALERT_EXCHANGE)
        disconnect_from_rabbit(self.test_email_alerts_handler.rabbitmq)
        ...
        self.test_api = None
        self.test_email_alerts_handler = None
        self.test_alerts_queue = None

    def test__str__returns_handler_name(self) -> None:
        self.assertEqual(self.test_handler_name,
                         str(self.test_email_alerts_handler))

    def test_handler_name_returns_handler_name(self) -> None:
        self.assertEqual(self.test_handler_name,
                         self.test_email_alerts_handler.handler_name)

    @mock.patch.object(RabbitMQApi, "start_consuming")
    def test_listen_for_data_calls_start_consuming(
            self, mock_start_consuming) -> None:
        mock_start_consuming.return_value = None
        self.test_email_alerts_handler._listen_for_data()
        mock_start_consuming.assert_called_once_with()

    def test_email_channel_returns_associated_email_channel(self) -> None:
        self.assertEqual(self.test_channel,
                         self.test_email_alerts_handler.email_channel)

    def test_alerts_queue_returns_the_alerts_queue(self) -> None:
        self.test_email_alerts_handler._alerts_queue = self.test_alerts_queue
        self.assertEqual(self.test_alerts_queue,
                         self.test_email_alerts_handler.alerts_queue)
    ...
    def test_init_initialises_handler_correctly(self) -> None:
        ...
        self.assertEqual(self.test_queue_size,
                         self.test_email_alerts_handler.alerts_queue.maxsize)
        self.assertEqual(self.test_max_attempts,
                         self.test_email_alerts_handler._max_attempts)
        self.assertEqual(
            self.test_alert_validity_threshold,
            self.test_email_alerts_handler._alert_validity_threshold)
    ...
    def test_initialise_rabbitmq_initialises_rabbit_correctly(
        ...
        try:
            ...
            self.test_email_alerts_handler.rabbitmq.queue_delete(
                self.test_email_alerts_handler._email_alerts_handler_queue)
            self.test_email_alerts_handler.rabbitmq.exchange_delete(
                HEALTH_CHECK_EXCHANGE)
            self.test_email_alerts_handler.rabbitmq.exchange_delete(
                ALERT_EXCHANGE)
            disconnect_from_rabbit(self.rabbitmq)

            self.test_email_alerts_handler._initialise_rabbitmq()
            ...
            # called successfully.
            self.assertTrue(
                self.test_email_alerts_handler.rabbitmq.is_connected)
            self.assertTrue(
                self.test_email_alerts_handler.rabbitmq.connection.is_open)
            self.assertTrue(
                self.test_email_alerts_handler.rabbitmq.channel
            ...
            # automatically.
            self.test_email_alerts_handler.rabbitmq.exchange_declare(
            ...
            res = self.test_email_alerts_handler.rabbitmq.queue_declare(
                self.test_email_alerts_handler._email_alerts_handler_queue,
    ...
    def test_send_heartbeat_sends_a_heartbeat_correctly(self) -> None:
        ...
        try:
            ...
            self.assertEqual(0, res.method.message_count)
            self.test_email_alerts_handler.rabbitmq.queue_bind(
                queue=self.test_rabbit_queue_name,
                exchange=HEALTH_CHECK_EXCHANGE,
                routing_key=HEARTBEAT_OUTPUT_WORKER_ROUTING_KEY)

            self.test_email_alerts_handler._send_heartbeat(self.test_heartbeat)
            ...
            _, _, body = self.test_email_alerts_handler.rabbitmq.basic_get(
                self.test_rabbit_queue_name)
    ...
    @mock.patch.object(EmailAlertsHandler, "_send_alerts")
    @mock.patch.object(EmailAlertsHandler, "_place_alert_on_queue")
    ...
    def test_process_alert_places_data_on_queue_if_no_processing_errors(
        ...
        try:
            self.test_email_alerts_handler._initialise_rabbitmq()
            blocking_channel = self.test_email_alerts_handler.rabbitmq.channel
            ...
            # Send alert
            self.test_email_alerts_handler._process_alert(blocking_channel,
                                                          method, properties,
                                                          body)
    ...
    def test_process_alert_raises_exception_if_sending_data_from_queue_error(
        ...
        try:
            ...
            self.assertRaises(error_class,
                              self.test_email_alerts_handler._process_alert,
    ...
    @mock.patch.object(Queue, "empty")
    @mock.patch.object(EmailAlertsHandler, "_send_heartbeat")
    @mock.patch.object(EmailAlertsHandler, "_send_alerts")
    @mock.patch.object(EmailAlertsHandler, "_place_alert_on_queue")
    @mock.patch.object(RabbitMQApi, "basic_ack")
    ...
    def test_place_alert_on_queue_places_alert_on_queue_if_queue_not_full(
        ...
        test_queue = Queue(3)
        self.test_email_alerts_handler._alerts_queue = test_queue
        test_queue.put('item1')
        test_queue.put('item2')

        self.test_email_alerts_handler._place_alert_on_queue(self.test_alert)
    ...
    def test_send_alerts_does_nothing_if_queue_is_empty(
        ...
        self.test_email_alerts_handler._send_alerts()
    ...
    @freeze_time("2012-01-01")
    @mock.patch.object(EmailChannel, "alert")
    def test_send_alerts_discards_old_alerts_and_sends_the_recent(
        ...
        self.assertTrue(self.test_email_alerts_handler.alerts_queue.empty())
    ...
    def test_send_alerts_stops_sending_if_an_alert_is_not_successfully_sent(
        ...
        self.assertFalse(self.test_email_alerts_handler.alerts_queue.empty())
        self.assertEqual(2, self.test_email_alerts_handler.alerts_queue.qsize())
        self.assertEqual(test_alert_1,
                         self.test_email_alerts_handler.alerts_queue.queue[0])
        self.assertEqual(test_alert_2,
                         self.test_email_alerts_handler.alerts_queue.queue[1])
