...
from src.utils.constants.rabbitmq import (
    ...
    DOCKERHUB_DT_INPUT_QUEUE_NAME, DOCKERHUB_TRANSFORMED_DATA_ROUTING_KEY,
    DOCKERHUB_RAW_DATA_ROUTING_KEY, HEARTBEAT_OUTPUT_WORKER_ROUTING_KEY)
...
class TestDockerHubDataTransformer(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.transformer_name = 'test_dockerhub_data_transformer'
        self.max_queue_size = 1000
        self.test_repo_namespace = 'test_namespace'
        ...
        self.test_repo = DockerHubRepo(self.test_repo_namespace,
                                       self.test_repo_name, self.test_repo_id,
        ...
        self.test_tags = {
            ...
            '5': {
                'tag_name': 'v0.1.0',
                'last_updated': 1622010453.11423
            }
        }

        # Set repository values
        self.test_repo.set_tags(self.test_tags)
        ...
        self.raw_data_example_result = {
            'result': {
                'meta_data': {
                    ...
                    'repo_namespace': self.test_repo_namespace,
                    ...
                    'time': self.test_last_monitored +
                            env.DOCKERHUB_MONITOR_PERIOD_SECONDS
                },
                'data': {
                    '0': {
                        'tag_name': 'v2.0.0',
        ...
        self.test_repo_new_metrics = DockerHubRepo(self.test_repo_namespace,
                                                   self.test_repo_name,
                                                   self.test_repo_id,
                                                   self.test_repo_parent_id)
        self.test_repo_new_metrics.set_last_monitored(
            self.test_last_monitored + env.DOCKERHUB_MONITOR_PERIOD_SECONDS)
        self.test_repo_new_metrics.set_tags(self.test_tags)

        td_meta_data_result = \
            self.transformed_data_example_result['result']['meta_data']
        td_data_result = \
            self.transformed_data_example_result['result']['data']
        self.test_data_for_alerting_result = {
            'result': {
                'meta_data': td_meta_data_result,
                'data': {
                    'current': self.test_repo_new_metrics.tags,
                    'previous': self.test_repo.tags
                }
            }
        }
        ...
        self.test_data_transformer = DockerHubDataTransformer(
    ...
    def test_initialise_rabbit_initialises_everything_as_expected(
        ...
        self.assertTrue(self.test_data_transformer.rabbitmq.channel.
        ...
        res = self.test_data_transformer.rabbitmq.queue_declare(
            DOCKERHUB_DT_INPUT_QUEUE_NAME, False, True, False, False)
    ...
    def test_load_state_successful_if_repo_exists_in_redis_and_redis_online(
        ...
        save_dockerhub_repo_to_redis(self.redis, self.test_repo)
        ...
        self.assertEqual(self.test_tags, loaded_repo.tags)
    ...
    def test_load_state_keeps_same_state_if_repo_in_redis_and_redis_offline(
        ...
        self.assertEqual(None, loaded_repo.tags)
    ...
    @mock.patch.object(DockerHubDataTransformer,
    ...
    @mock.patch.object(DockerHubDataTransformer, "_transform_data")
    ...
    def test_process_raw_data_updates_state_if_no_processing_errors_new_repo(
        ...
        self.assertEqual(self.test_data_str,
                         self.test_data_transformer._state['repo2'])
        expected_data = DockerHubRepo(self.test_repo_namespace,
                                      self.test_repo_name,
                                      self.test_repo_id,
                                      self.test_repo_parent_id)
        self.assertEqual(
    ...
    @mock.patch.object(DockerHubDataTransformer, "_transform_data")
    @mock.patch.object(DockerHubDataTransformer, "_place_latest_data_on_queue")
    @mock.patch.object(RabbitMQApi, "basic_ack")
    ...
    @mock.patch.object(DockerHubDataTransformer, "_update_state")
    @mock.patch.object(DockerHubDataTransformer, "_place_latest_data_on_queue")
    @mock.patch.object(RabbitMQApi, "basic_ack")
    ...
    @mock.patch.object(DockerHubDataTransformer, "_send_data")
    @mock.patch.object(RabbitMQApi, "basic_ack")
    ...
    @mock.patch.object(DockerHubDataTransformer, "_send_heartbeat")
    @mock.patch.object(DockerHubDataTransformer, "_send_data")
    @mock.patch.object(RabbitMQApi, "basic_ack")
