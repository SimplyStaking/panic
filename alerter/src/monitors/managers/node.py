...
from src.utils.constants.rabbitmq import (
    ...
    NODES_CONFIGS_ROUTING_KEY_CHAINS, GETH_NODES_CONFIGS_ROUTING_KEY_CHAINS,
    TOPIC)
...
class NodeMonitorsManager(MonitorsManager):
    ...
    def _initialise_rabbitmq(self) -> None:
        ...
        self.logger.info("Binding queue '%s' to exchange '%s' with routing "
                         "key '%s'", NODE_MON_MAN_CONFIGS_QUEUE_NAME,
                         CONFIG_EXCHANGE, GETH_NODES_CONFIGS_ROUTING_KEY_CHAINS)
        self.rabbitmq.queue_bind(NODE_MON_MAN_CONFIGS_QUEUE_NAME,
                                 CONFIG_EXCHANGE,
    ...
    def _process_geth_node_configs(self, sent_configs: Dict,
                                   current_configs: Dict) -> Dict:
        """
        This function processes the new geth nodes configs for a particular
        chain. It creates/removes new processes, and it keeps a local copy of
        all changes being done step by step. This is done in this way so that if
        a sub-config is malformed or a process errors, we can keep track of what
        the new self._nodes_config should be.
        :param sent_configs:
        The new geth node configurations for a particular chain
        :param current_configs:
        The currently stored geth nodes configurations for a particular chain
        :return:
        The new geth nodes configuration for a particular chain based on the
        ...
        try:
            ...
            for config_id in new_configs:
                ...
                node_config = GethNodeConfig(node_id, parent_id, node_name,
    ...
    def _process_configs(
        ...
        updated_configs = {}
        if config_type == 'geth_nodes_config':
            updated_configs = self._process_geth_node_configs(
