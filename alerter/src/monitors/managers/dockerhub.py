...
from src.utils.constants.rabbitmq import (
    CONFIG_EXCHANGE, HEALTH_CHECK_EXCHANGE, DH_MON_MAN_CONFIGS_QUEUE_NAME,
    DH_MON_MAN_HEARTBEAT_QUEUE_NAME, PING_ROUTING_KEY,
    DH_MON_MAN_CONFIGS_ROUTING_KEY_CHAINS, DH_MON_MAN_CONFIGS_ROUTING_KEY_GEN,
...
class DockerHubMonitorsManager(MonitorsManager):
    ...
    def _initialise_rabbitmq(self) -> None:
        ...
        self.rabbitmq.exchange_declare(HEALTH_CHECK_EXCHANGE, TOPIC, False,
                                       True, False, False)
        self.logger.info("Creating queue '%s'", DH_MON_MAN_HEARTBEAT_QUEUE_NAME)
        self.rabbitmq.queue_declare(DH_MON_MAN_HEARTBEAT_QUEUE_NAME, False,
                                    True, False, False)
        self.logger.info("Binding queue '%s' to exchange '%s' with routing key "
                         "'%s'", DH_MON_MAN_HEARTBEAT_QUEUE_NAME,
                         HEALTH_CHECK_EXCHANGE, PING_ROUTING_KEY)
        self.rabbitmq.queue_bind(DH_MON_MAN_HEARTBEAT_QUEUE_NAME,
                                 HEALTH_CHECK_EXCHANGE, PING_ROUTING_KEY)
        self.logger.debug("Declaring consuming intentions on '%s'",
                          DH_MON_MAN_HEARTBEAT_QUEUE_NAME)
        self.rabbitmq.basic_consume(DH_MON_MAN_HEARTBEAT_QUEUE_NAME,
                                    self._process_ping, True, False, None)

        self.logger.info("Creating exchange '%s'", CONFIG_EXCHANGE)
        self.rabbitmq.exchange_declare(CONFIG_EXCHANGE, TOPIC, False, True,
                                       False, False)
        self.logger.info("Creating queue '%s'", DH_MON_MAN_CONFIGS_QUEUE_NAME)
        self.rabbitmq.queue_declare(DH_MON_MAN_CONFIGS_QUEUE_NAME, False, True,
                                    False, False)
        self.logger.info("Binding queue '%s' to exchange '%s' with routing key "
                         "'%s'", DH_MON_MAN_CONFIGS_QUEUE_NAME,
                         CONFIG_EXCHANGE, DH_MON_MAN_CONFIGS_ROUTING_KEY_CHAINS)
        self.rabbitmq.queue_bind(DH_MON_MAN_CONFIGS_QUEUE_NAME, CONFIG_EXCHANGE,
                                 DH_MON_MAN_CONFIGS_ROUTING_KEY_CHAINS)
        self.logger.info("Binding queue '%s' to exchange '%s' with routing "
                         "key '%s'", DH_MON_MAN_CONFIGS_QUEUE_NAME,
                         CONFIG_EXCHANGE, DH_MON_MAN_CONFIGS_ROUTING_KEY_GEN)
        self.rabbitmq.queue_bind(DH_MON_MAN_CONFIGS_QUEUE_NAME, CONFIG_EXCHANGE,
                                 DH_MON_MAN_CONFIGS_ROUTING_KEY_GEN)
        self.logger.debug("Declaring consuming intentions on '%s'",
                          DH_MON_MAN_CONFIGS_QUEUE_NAME)
        self.rabbitmq.basic_consume(DH_MON_MAN_CONFIGS_QUEUE_NAME,
    ...
    def _create_and_start_monitor_process(
            self, repo_config: DockerHubRepoConfig, config_id: str, chain:
            str, base_chain: str, sub_chain: str) -> None:
        log_and_print("Creating a new process for the monitor of {}"
                      .format(repo_config.repo_name), self.logger)
        process = multiprocessing.Process(target=start_dockerhub_monitor,
                                          args=(repo_config,))
    ...
    def _process_configs(
        ...
        if method.routing_key == DH_MON_MAN_CONFIGS_ROUTING_KEY_GEN:
        ...
        try:
            ...
            for config_id in new_configs:
                ...
                repo_namespace = config['repo_namespace']

                repo_name = config['repo_name']

                monitor_repo = str_to_bool(config['monitor_repo'])
                tags_page = env.DOCKERHUB_TAGS_TEMPLATE.format(repo_namespace,
                                                               repo_name)
                ...
                repo_config = DockerHubRepoConfig(repo_id, parent_id,
                                                  repo_namespace, repo_name,
                                                  monitor_repo, tags_page)
            ...
            for config_id in modified_configs:
                ...
                old_repo_namespace = \
                    modified_configs[config_id]['repo_namespace']
                old_repo_name = modified_configs[config_id]['repo_name']

                # If we should not monitor the repo, delete the previous process
                # from memory and move on to the next config
                if not monitor_repo:
        ...
        try:
            ...
            for config_id, process_details in self.config_process_dict.items():
                ...
                if process.is_alive():
                    ...
                    monitor_repo = str_to_bool(config['monitor_repo'])
                    tags_page = \
                        env.DOCKERHUB_TAGS_TEMPLATE.format(repo_namespace,
                                                           repo_name)
                    repo_config = DockerHubRepoConfig(repo_id, parent_id,
                                                      repo_namespace, repo_name,
                                                      monitor_repo, tags_page)
