...
class DockerhubStore(Store):
    ...
    def _initialise_rabbitmq(self) -> None:
        ...
        Declares a queue named `dockerhub_store_input_queue` and binds it to the
        store exchange with a routing key `transformed_data.dockerhub` meaning
        anything coming from the transformer with regards to dockerhub updates
        ...
        self.rabbitmq.exchange_declare(exchange=STORE_EXCHANGE,
                                       ...
                                       internal=False)
        self.rabbitmq.queue_declare(DOCKERHUB_STORE_INPUT_QUEUE_NAME,
                                    passive=False, durable=True,
                                    exclusive=False, auto_delete=False)
        self.rabbitmq.queue_bind(
            queue=DOCKERHUB_STORE_INPUT_QUEUE_NAME, exchange=STORE_EXCHANGE,
    ...
    def _listen_for_data(self) -> None:
        self.rabbitmq.basic_consume(
            queue=DOCKERHUB_STORE_INPUT_QUEUE_NAME,
    ...
    def _process_data(self,
        ...
        """
        dockerhub_data = json.loads(body)
        self.logger.debug("Received %s. Now processing this data.",
                          dockerhub_data)

        processing_error = False
        try:
            self._process_redis_store(dockerhub_data)
        except KeyError as e:
            self.logger.error("Error when parsing %s.", dockerhub_data)
            self.logger.exception(e)
            processing_error = True
        except ReceivedUnexpectedDataException as e:
            self.logger.error("Error when processing %s", dockerhub_data)
            self.logger.exception(e)
            processing_error = True
    ...
    def _process_redis_result_store(self, data: Dict) -> None:
        ...
        self.logger.debug(
            "Saving %s/%s state: _tags=%s, _last_monitored=%s", repo_namespace,
            repo_name, json.dumps(metrics['tags']),
