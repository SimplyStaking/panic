...
def _initialise_store_logger(store_name: str) -> logging.Logger:
...
def _initialise_system_store() -> SystemStore:
    store_name = 'System Store'

    store_logger = _initialise_store_logger(store_name)

    # Try initializing the system store until successful
    while True:
        try:
            system_store = SystemStore(store_name, store_logger)
            log_and_print("Successfully initialised {}".format(store_name),
                          store_logger)
            break
        except Exception as e:
            msg = get_initialisation_error_message(store_name, e)
...
def _initialise_github_store() -> GithubStore:
...
def _initialise_alert_store() -> AlertStore:
...
def start_system_store() -> None:
    system_store = _initialise_system_store()
    start_store(system_store)


def start_github_store() -> None:
    github_store = _initialise_github_store()
    start_store(github_store)


def start_alert_store() -> None:
    alert_store = _initialise_alert_store()
