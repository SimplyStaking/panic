...
class DockerhubAlerterManager(AlertersManager):
    ...
    def _initialise_rabbitmq(self) -> None:
        ...
        self.logger.info("Creating queue '%s'",
                         DH_ALERTERS_MAN_HEARTBEAT_QUEUE_NAME)
        self.rabbitmq.queue_declare(DH_ALERTERS_MAN_HEARTBEAT_QUEUE_NAME, False,
                                    True, False, False)
        self.logger.info("Binding queue '%s' to exchange '%s' with routing "
                         "key '%s'", DH_ALERTERS_MAN_HEARTBEAT_QUEUE_NAME,
                         HEALTH_CHECK_EXCHANGE, PING_ROUTING_KEY)
        self.rabbitmq.queue_bind(DH_ALERTERS_MAN_HEARTBEAT_QUEUE_NAME,
                                 HEALTH_CHECK_EXCHANGE, PING_ROUTING_KEY)
        self.logger.info("Declaring consuming intentions on '%s'",
                         DH_ALERTERS_MAN_HEARTBEAT_QUEUE_NAME)
        self.rabbitmq.basic_consume(DH_ALERTERS_MAN_HEARTBEAT_QUEUE_NAME,
    ...
    def _start_alerters_processes(self) -> None:
        """
        Start the DockerHub Alerter in a separate process if it is not yet
        started or it is not alive. This must be done in case of a restart of
        the manager.
        """
        if DOCKERHUB_ALERTER_NAME not in self.alerter_process_dict or not \
                self.alerter_process_dict[DOCKERHUB_ALERTER_NAME].is_alive():
            """
            ...
            achieve this.
            """
            alert = ComponentResetAlert(DOCKERHUB_ALERTER_NAME,
            ...
            log_and_print("Attempting to start the {}.".format(
                DOCKERHUB_ALERTER_NAME), self.logger)
            dockerhub_alerter_process = Process(target=start_dockerhub_alerter,
                                                args=())
            dockerhub_alerter_process.daemon = True
            dockerhub_alerter_process.start()
            self._alerter_process_dict[
                DOCKERHUB_ALERTER_NAME] = dockerhub_alerter_process
    ...
    def _on_terminate(self, signum: int, stack: FrameType) -> None:
        log_and_print("{} is terminating. Connections with RabbitMQ will be "
                      "closed, and any running dockerhub alerters will be "
    ...
    # The DockerHub Alerters Manager does not listen for configs
