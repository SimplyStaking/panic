...
| `/mute` | List of severities, for example: `/mute INFO CRITICAL`) | Mutes List(<severity>) alerts on all channels (Including all other channels which are set-up, for example Opsgenie) for the chains associated with the Telegram Channel. If the list of severities is not given, all alerts for the chains associated with the Telegram Channel are muted on all channels. |
