...
| `/mute` | List of severities, for example: `/mute INFO CRITICAL` | Let `INFO` and `CRITICAL` be the severities passed as parameters to the `/mute` command. The `/mute` command mutes `INFO` and `CRITICAL` alerts on all channels (Including all other channels which are set-up, for example Opsgenie) for the chains associated with the Telegram Channel. If the list of severities is not given, all alerts for the chains associated with the Telegram Channel are muted on all channels. |
...
| `/muteall` | List of severities, for example: `/muteall INFO CRITICAL` | Let `INFO` and `CRITICAL` be the severities passed as parameters to the `/muteall` command. The `/muteall` command mutes `INFO` and `CRITICAL` alerts on all channels (Including all other channels which are set-up, for example Opsgenie) for every chain being monitored (including the GENERAL chain). If the list of severities is not given, all alerts for all chains being monitored are muted on all channels. |
| `/unmuteall` | None | Unmutes all alert severities on all channels (Including all other channels which are set-up ex. Opsgenie) for every chain being monitored (including the GENERAL chain). |
