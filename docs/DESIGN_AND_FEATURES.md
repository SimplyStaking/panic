...
| `/mute` | `Optional( List(<severity>) )` (*Example*: `/mute INFO CRITICAL`) | Mutes List(<severity>) alerts on all channels (Including all other channels which are set-up, for example Opsgenie) for the chains associated with the Telegram Channel. If the list of severities is not given, all alerts for the chains associated with the Telegram Channel are muted on all channels. |
...
| `/muteall` | `Optional( List(<severity>) )` (*Example*: `/muteall INFO CRITICAL`) | Mutes List(<severity>) alerts on all channels (Including all other channels which are set-up, for example Opsgenie) for every chain being monitored (including the GENERAL chain). If the list of severities is not given, all alerts for all chains being monitored are muted on all channels. |
