...
class LoadConfig extends Component {
  ...
  async loadConfigs() {
    const {
      ...
      loadTwilioDetails,
      loadEmailDetails,
      loadPagerdutyDetails,
      loadOpsgenieDetails,
      loadNodeCosmosDetails,
      loadReposCosmosDetails,
      loadRepositoryDetails,
      loadKMSCosmosDetails,
      loadKMSDetails,
        ...
        if (res[1] === 'general') {
          ...
          if (res[2] === 'email_config.ini') {
            ...
            Object.keys(config.data.result).forEach(function (key) {
              ...
              if (payload.twilio_phone_numbers_to_dial.length === 0) {
                ...
                payload.twilio_phone_numbers_to_dial = payload.twilio_phone_numbers_to_dial.split(
                  ',',
                );
              }
              if (payload.parent_ids.length === 0) {
          ...
          if (res[2] === 'cosmos') {
            if (res[4] === 'nodes_config.ini') {
              ...
              thresholdAlerts = JSON.parse(
                JSON.stringify(cosmosThresholdAlerts),
              );
              timeWindowAlerts = JSON.parse(
                JSON.stringify(cosmosTimeWindowAlerts),
              );
              ...
              Object.keys(config.data.result).forEach(function (key) {
                ...
                if (key in repeatAlerts.byId) {
                  ...
                  critical = {
                    repeat: config.data.result[key].critical_repeat,
                    enabled:
                      config.data.result[key].critical_enabled === 'true',
                  };
                  repeatAlerts.byId[key].warning = warning;
                  repeatAlerts.byId[key].critical = critical;
                  repeatAlerts.byId[key].enabled = config.data.result[key].enabled === 'true';
                  ...
                  timeWindowAlerts.byId[key].enabled = config.data.result[key].enabled === 'true';
                  ...
                  severityAlerts.byId[key].enabled = config.data.result[key].enabled === 'true';
                }
              });
              payload = {
                chain_name: res[3],
            ...
            if (res[4] === 'nodes_config.ini') {
              ...
              thresholdAlerts = JSON.parse(
                JSON.stringify(substrateThresholdAlerts),
              );
              timeWindowAlerts = JSON.parse(
                JSON.stringify(substrateTimeWindowAlerts),
              );
              severityAlerts = JSON.parse(
                JSON.stringify(substrateSeverityAlerts),
              );
              // eslint-disable-next-line
              Object.keys(config.data.result).forEach(function (key) {
                parent_id = config.data.result[key].parent_id;
